/**
 * DSH Webview 提供者 - 重构简化版本
 * 负责在侧边栏显示 DeepSeek Harness Web 界面
 */
...
import { getEventManager, DshEventManager } from '../../core/EventManager';
import { EventType } from '../../core/eventTypes';
import { DshStatus } from '../../core/types';
 ...
 * Webview 视图类型常量
 */
const WEBVIEW_VIEW_TYPE = 'dshWebview';
 ...
 * DSH Webview 提供者类
 */
export class DshWebviewProvider implements vscode.WebviewViewProvider, IWebviewProvider {
  // === Webview 引用 ===
  ...
  private eventManager: DshEventManager;

  constructor(
    private context: vscode.ExtensionContext,
    private configurationService: ConfigurationService,
    private dshManager: DshManager,
    private onOpenInBrowser: () => void,
    private onToggleSidebar: () => void
  ) {
    this.outputChannel = vscode.window.createOutputChannel('DSH Webview');
  ...
  private async silentHealthCheck(): Promise<void> {
    try {
      const connected = await this.dshManager.checkConnection(2000);
      if (connected) {
        this.log('静默健康检查成功：服务正常，保持 ready');
      } else {
  ...
  private async checkStatusWithTimeout(timeout: number): Promise<{
    ...
    const statusPromise = this.dshManager.getStatus();

    const timeoutPromise = new Promise<DshStatus>((resolve) => {
      setTimeout(() => resolve(DshStatus.NotRunning), timeout);
    });
    ...
    switch (status) {
      ...
      case DshStatus.NotInstalled: {
        // 展示具体原因（Node 未安装 / 版本过低 / dsh 未安装）
        const reason = await this.dshManager.getNotInstalledReason();
        return { state: 'notInstalled', message: reason };
      }
      case DshStatus.NotRunning:
      default: {
        const hasExternal = await this.dshManager.hasExternalDshProcess();
        if (hasExternal) {
          return { state: 'ready', message: '' };
        }
  ...
  private async startDsh(): Promise<void> {
    try {
      ...
      const success = await this.dshManager.startInBackground();

      if (success) {
        this.log('DSH 启动命令已发送，开始健康检查...');
        this.setState('loading', l10n.t('status.waiting'));
        await this.healthCheckPolling();
      } else {
        this.log('DSH 启动命令失败，尝试健康检查...');
        this.setState('loading', l10n.t('status.waiting'));
        await this.healthCheckPolling();
      }
    } catch (error) {
  ...
  private async connectToExternalProcess(): Promise<void> {
    ...
    const isConnected = await this.dshManager.checkConnection(2000);
  ...
  private async healthCheckPolling(): Promise<void> {
    ...
    this.timers.poll = setInterval(async () => {
      ...
      try {
        const healthy = await this.dshManager.checkConnection(2000);
  ...
  private async checkAndNotifyConnection(): Promise<void> {
    const connected = await this.dshManager.checkConnection();
  ...
  private postMessageToWebview(message: any): void {
    try {
      ...
      this.log(`发送消息到 Webview 失败: ${error}`);
    }
  }
  ...
  /**
   * 获取 DSH Web UI URL
   * dsh Web UI 无需路径编码，直接访问根路径
   */
  private getDshUrl(): string {
  ...
  <script>
    ...
    try {
      ...
      function t(key, ...args) {
        ...
        return value.replace(/{([0-9]+)}/g, (_, index) => args[index] ?? '');
      }
    } catch (error) {
    ...
    const SAVED_STATE_KEY = 'dshState';
    const STATE_EXPIRY_MS = ${STATE_EXPIRY_MS};
    ...
    function setState(state, message) {
      ...
      if (state === 'ready') {
        container.innerHTML = \`<iframe src="\${url}" frameborder="0" id="dshFrame"></iframe>\`;
      } else if (state === 'error') {
        container.innerHTML = \`
        ...
        if (startBtn) {
          startBtn.addEventListener('click', () => {
            ...
            startBtn.innerHTML = \`
              ...
              \${t('button.starting')}
            \`;
            vscode.postMessage({ type: 'startDsh' });
          });
        }
      } else if (state === 'idle') {
        container.innerHTML = \`
...
<body>
  ...
  public async refreshWebview(): Promise<void> {
    ...
    try {
      const isConnected = await this.dshManager.checkConnection(5000);

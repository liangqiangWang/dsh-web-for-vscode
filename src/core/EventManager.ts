 ...
 */
export class DshEventManager extends EventEmitter {
  private static instance: DshEventManager;
   ...
   * 获取事件管理器单例实例
   */
  public static getInstance(): DshEventManager {
    if (!DshEventManager.instance) {
      DshEventManager.instance = new DshEventManager();
    }
    return DshEventManager.instance;
  }
...
// 导出单例获取函数
export function getEventManager(): DshEventManager {
  return DshEventManager.getInstance();
}

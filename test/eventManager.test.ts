...
test('processStateChanged 事件收发', () => {
  ...
  let received: { status?: DshStatus; timestamp?: number } = {};
  em.onProcessStateChanged((data) => {
    received = data;
  });
  em.emitProcessStateChanged({ status: DshStatus.Running, timestamp: 123 });
  assert.equal(received.status, DshStatus.Running);
...
test('removeAllListenersForEvent 移除后不再触发', () => {
  ...
  em.removeAllListenersForEvent(EventType.ProcessStateChanged);
  em.emitProcessStateChanged({ status: DshStatus.Running, timestamp: 1 });
